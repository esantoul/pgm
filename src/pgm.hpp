...
#define CPP20_IMPL
  ...
  constexpr bool Condition_Helper(std::index_sequence<arg_idx...>)
  {
    ...
    return std::is_invocable_v<Callable_t, std::tuple_element_t<arg_idx, Args_tuple_t>...> &&
      (std::is_integral_v<result_t> || std::is_enum_v<result_t>);
  }
...
#define CPP17_IMPL
  ...
  struct is_task<Callable_t, Ret_t(Args_t...)> { static constexpr bool value = std::is_invocable_r_v<std::conditional_t<std::is_same_v<Ret_t, bool>, bool, std::optional<Ret_t>>, Callable_t, Args_t...>; };
  ...
  struct is_condition;
  template <typename Callable_t, typename Ret_t, typename ... Args_t>
  struct is_condition<Callable_t, Ret_t(Args_t...)> { static constexpr bool value = 
                                                      std::is_invocable_v<Callable_t, Args_t...> &&
                                                      (std::is_integral_v<std::invoke_result_t<Callable_t, Args_t...>> ||
                                                        std::is_enum_v<std::invoke_result_t<Callable_t, Args_t...>>);
  };
...
#error Program must be compiled with C++17 or more recent
  ...
  public:
    ...
    struct task_ret<Ret_t(Args_t...)> { using type = std::conditional_t<std::is_same_v<Ret_t, bool>, bool, std::optional<Ret_t>>; };
    ...
    struct args_tuple<Ret_t(Args...)> { using type = std::tuple<Args...>; };
    ...
    struct cond_ret<Callable_t, Ret_t(Args...)> { using type = std::invoke_result_t<Callable_t, Args...>; };
  ...
  private:
    ...
    constexpr Helper::task_ret_t<Proto_t> execute_tasks(Args_tuple_t targs, std::index_sequence<idx...>) const
    {
      ...
      if (((ret = std::apply(std::get<idx>(mT), targs)) || ...)) void();
      return ret;
    }
...
#endif
    constexpr Switcher(Prototype<Proto_t>, CondFun_t cond, DefFun_t def, std::pair<Helper::cond_ret_t<CondFun_t, Proto_t>, CaseFun_t>... cases) : mCFun{cond}, mDFun{def}, mCVals{cases.first...}, mCFuns{cases.second...} {}
...
public:
  ...
  {
    return InterpreterHelper<Proto_t, CaseTuple_t>(cf, [](auto...) -> pgm::Helper::task_ret_t<Proto_t> { return {}; }, std::make_index_sequence<std::tuple_size_v<CaseTuple_t>>());
  }
private:
...
constexpr auto mycond = [](const uint8_t* bytes, std::size_t length) -> std::size_t { return length; };
constexpr auto mydefault = [](const uint8_t* bytes, std::size_t length) -> std::optional<int> { if (length < 12) return length; return {}; };
...
constexpr auto mySwitcher =
  SwitcherMaker::Interpreter<int(const uint8_t*, std::size_t), CaseList>(
    mycond,
    mydefault
);

constexpr auto p = pgm::Process<int(const uint8_t*&, std::size_t&)>{}
  << [](const uint8_t* bytes, std::size_t length) -> std::optional<int> { if (length < 2) return -1; return {}; }
